...
export const InfoMutate = ({ swab }: SwabDataProps) => {
    ...
    const performedType = useWatch({
        control,
        compute: (data: UpdateSwabType) => {
            const typeSwabBD = // vem de fora, prop ou contexto
        
        const originalIsAtpOrMicro = ATP_REQUIRED_TYPES.includes(typeSwabBD)
            const currentIsVisual = data.performedType === SwabCheckType.VISUAL

            // Só mostra se: era ATP/MICRO e mudou pra VISUAL
            return originalIsAtpOrMicro && currentIsVisual
        }
    })
