...
export const InfoMutate = ({ swab }: SwabDataProps) => {
    ...
    // exige justificativa quando a troca é feita agora (tipo cadastrado exige ATP e usuário mudou pra VISUAL)
    const isTypeChangeRequiringJustification = useWatch({
        control,
        compute: (data: UpdateSwabType) =>
            ATP_REQUIRED_TYPES.includes(swab.check.type) &&
            data.performedType === SwabCheckType.VISUAL
    })

    // mostra o campo se a mudança exige justificativa agora, OU se já existe uma justificativa salva pra poder ver/editar
    const showUpdateJustification = useWatch({
        control,
        compute: (data: UpdateSwabType) =>
            isTypeChangeRequiringJustification || !!data.updateSwabJustification?.trim()
    })

    // idem para justificativa de mesma torneira
    const showSameFaucetJustification = useWatch({
        control,
        compute: (data: UpdateSwabType) =>
            isSameFaucet || !!data.sameFaucetJustification?.trim()
    })
    ...
    // (só depois da primeira renderização, pra não apagar valor já salvo ao abrir o form)
    useEffect(() => {
    ...
    useEffect(() => {
        if (isFirstRender.current) return
        if (!showSameFaucetJustification) {
            resetField("sameFaucetJustification")
        }
    }, [showSameFaucetJustification, resetField])
