...
import { Calendar, ResultSelect, TypeSwabSelect, ValueAtp } from "@/app/(private)/swab/components/index";
...
export const InfoMutate = ({ swab }: SwabDataProps) => {
    ...
    const typesObervations = ["performedType", "batch"]
    //Aqui estou oberservando o campo do tipo de atp/Micro, caso seja visual bloqueio a entrada do value atp
    const disableInputValueATP: boolean = useWatch({
        control,
        name: typesObervations.map(item=>item.),
        compute: (data: SwabCheckType) => {
            if (data === SwabCheckType.VISUAL) {
                resetField("valueAtp")
                return true
            }
            return false
