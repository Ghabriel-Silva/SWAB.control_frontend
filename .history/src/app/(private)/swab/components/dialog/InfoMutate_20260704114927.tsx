...
export const InfoMutate = ({ swab }: SwabDataProps) => {
    ...
    const showUpdateJustification = useWatch({
        control,
        compute: (data: UpdateSwabType) =>
            ATP_REQUIRED_TYPES.includes(swab.check.lastType ?? swab.check.type) &&
            data.performedType === SwabCheckType.VISUAL
    })

    const showUpdateJustificationWhenExisteLastType =
        !showUpdateJustification && swab.check.lastType
            ? true
            : false
            ...
            <form onSubmit={handleSubmit(OnSubmit)} noValidate>
                ...
                >
                    <FormField label="Tipo Swab" error={errors.performedType?.message}>
                        <HStack>
                            <TypeSwabSelect />
                            {showUpdateJustificationWhenExisteLastType && (
                                <Justification
