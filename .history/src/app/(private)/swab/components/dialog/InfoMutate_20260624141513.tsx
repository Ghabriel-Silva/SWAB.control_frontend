...
export const InfoMutate = ({ swab }: SwabDataProps) => {
    ...
    const isSameFaucet = useWatch({
        control,
        compute: data => data.faucetCode === swab.lastFaucetLocation
    })
    ...
    const oberservationType = useWatch({
        control,
        compute: (data: UpdateSwabType) =>
            data.performedType !== swab.check.type
    })

    const updateSwabJustification = useWatch({
        control,
        name: 'updateSwabJustification'
    })

    // useEffect(() => {
    //     if (updateSwabJustification?.trim()) {
    //         trigger('performedType')
    //     }
    // }, [updateSwabJustification, trigger])
