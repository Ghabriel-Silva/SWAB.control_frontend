        ...
        },
            {
                onSuccess: (data) => {
                    if (data.data.pending.length || data.data.invalidTanks.length) {
                        const pendente = data.data.pending.map(item => item.tank)
                        const invalidos = data.data.invalidTanks

                        toaster.create({
                            title: pendente ? 'Pendente' : invalidos ? 'Invalidos' : 'Pendentes/Invalidos',
                            description: pendente ? `Swab ${pendente} consta como pendente no sistema`
                                : invalidos ? `${invalidos} não estão cadastrado no sistema`
                                    : 'Swab ${pendente} consta como pendente no sistema & ${invalidos} não estão cadastrado no sistema',
                            closable: true,
                            duration: 6000,
                            type: "warning"
                        })
                    ...
                    //     toaster.create({
                    //         title: "Invalidos",
                    //         description: `${invalidos} não estão cadastrado no sistema`,
                    //         closable: true,
                    //         duration: 6000,
                    //         type: "warning"
                    //     })
