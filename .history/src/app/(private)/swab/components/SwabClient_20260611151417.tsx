...
import { FullScreenLoading, SubtitleText } from "@/app/(private)/components/index"
...
export default function SwabClient() {
    ...
    const status = searchParams.get("result") ?? "PENDING";
    const page = Number(searchParams.get("page") ?? 1);
    const limit = Number(searchParams.get("limit") ?? 20);

    const { data, isLoading, isError, error } = useGetSwab(params:{
