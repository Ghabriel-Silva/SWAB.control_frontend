...
export const updateSwabSchema = yup.object({
    ...
    valueAtp: yup
        ...
        .when('performedType', ([performedType], schema) => {
            if (ATP_REQUIRED_TYPES.includes(performedType)) {
                return schema.required('Informe o Valor do ATP (RLU)')
            }
            // performedType é VISUAL -> força null sempre
    ...
    batch: yup
        ...
        .when('performedType', ([performedType], schema) => {
            if (ATP_REQUIRED_TYPES.includes(performedType)) {
                return schema.required('O lote ATP é obrigatório')
            }
            if (performedType === SwabCheckType.VISUAL) {
                return schema.transform(() => null).nullable().default(null)
            }
            return schema.transform(() => null).default(null)
        }),
    result: yup
