...
import { FORBIDDEN_ROUTE, isRouteAllowed, UNAUTHENTICATED_ROUTE } from "./permissions/routes.config";
...
export async function proxy(request: NextRequest) {
    ...
    if (authToken && !publicRoute) {
        try {
            ...
            const session = verified.payload as unknown as MyJwtPayload


            //checa se o role do usuario pode acessar a rota
            if (!isRouteAllowed(path, session.role)) {
                return redirectTo(request, FORBIDDEN_ROUTE)
            }
            return NextResponse.next()

        } catch {
            // token inválido ou expirado limpa cookie e manda pro login
            const response = redirectTo(request, UNAUTHENTICATED_ROUTE)
